...
log(
	...
	async getTweets(
		...
		console.log(
			`[CrawlerWorker] Crawled ranges from database for ${keyword}: ${crawledRanges.from} to ${crawledRanges.to}`)
		console.log(
		...
		if (nestedIndex === 0) {
			const overlapRanges =
				...
				if(crawledRanges.from && crawledRanges.to) overlapRanges.push(crawledRanges);
