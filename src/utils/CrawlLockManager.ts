...
export default class CrawlerLock extends LockManager {
  ...
  async checkStartDateEndDateContainsOnKeys(keyword, start, end): Promise<any> {
    ...
    for (const range of ranges) {
      ...
      if (
        ...
        dateOverlaps.push({
          from: range.start ? rangeStart > startDate ? range.start : start : start,
          to: range.end ? rangeEnd < endDate ? range.end : end : end
        });
  ...
  public splitAndRemoveOverlappingRanges(ranges: DateRange, keys:any): DateRange[] {
    const result = [];
    const request = {
      start: ranges.start,
      end: ranges.end,
    ...
    const getNextDay = (date: string): string => {
      ...
      // If current range overlaps with our request
			if (range.from <= request.end && range.to >= currentStart) {
				// Add segment before overlap (if any)
				if (currentStart < range.from) {
					result.push({
						start: currentStart,
						end: getPreviousDay(range.from),
					});
				}
				// Move start to after this range
				currentStart = getNextDay(range.to);
			}
		}
